...
public class Exercise4 {
    ...
    // Find the largest negative number in the 2D array:
    public static void largestNegativeNumber(int a[][], int numRows, int numCols) {
        int largestNegative = -50;
        for (int i = 0; i < numRows; i++) {
            for (int j = 0; j < numCols; j++) {
                if (a[i][j] < 0 && a[i][j] > largestNegative) {
                    largestNegative = a[i][j];
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        largestNegativeNumber(a, numRows, numCols);
    }
}
